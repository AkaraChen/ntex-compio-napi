#!/bin/bash
# Extreme sweep: find where each stack tops out, and record what it costs.
#
# Differences from run-bench.sh:
#   * concurrency swept to 2048, so the fd limit is raised explicitly
#   * CPU + RSS of BOTH the server and the load generator are sampled per run,
#     so the report can say which side saturated instead of guessing
#   * bare runtimes (node http, Bun.serve) are included as ceilings, so
#     "framework X is faster" can be read against "the runtime allows Y"
set -u
...
CONNS="${CONNS:-1 32 128 512 1024 2048}"
...
# 2048 concurrent connections needs more than the default 1024 soft limit
...
start_server() { # name port -> SRV_PID SRV_LOG, waits for READY
  local name="$1" port="$2"
  SRV_LOG="$OUT/server-$name.log"
  # A leftover server from an aborted run silently squats the port and turns the
  # whole cell into a measurement of ITSELF. Fail loudly instead.
  if ss -ltn 2>/dev/null | grep -q ":$port "; then
    echo "PORT $port ALREADY IN USE - refusing to run $name against a stray server"
    return 1
  fi
  : > "$SRV_LOG"
  case "$name" in
...
run_cell() { # target route conn rep port
  ...
  # sanitise the route for the filename: "/users/42" would otherwise be written
  # into a non-existent subdirectory and silently produce an empty result
  ...
  local oha_pid=$!
  python3 "$BENCH/sampler.py" "$SRV_PID,$oha_pid" "${base}.res.json" 0.1 &
  local samp_pid=$!
...
d = load(oha_path)[0] if isinstance(load(oha_path), list) else load(oha_path)
try:
...
vals = list(r.values())
sc = vals[0]["cpu_cores"] if len(vals) > 0 and vals[0].get("cpu_cores") is not None else "NA"
srss = vals[0]["peak_rss_mb"] if len(vals) > 0 else "NA"
cc = vals[1]["cpu_cores"] if len(vals) > 1 and vals[1].get("cpu_cores") is not None else "NA"
def ms(v):
...
print(f"  {target:10s} {route:9s} c={conn:<5s} r={rep}  rps={rps if rps=='NA' else round(rps)}  srv_cores={sc if sc=='NA' else round(sc,2)}  cli_cores={cc if cc=='NA' else round(cc,2)}")
PY
}

echo "=== EXTREME sweep: frameworks vs bare runtimes ==="
echo "duration=${DUR}s warmup=${WARMUP}s reps=${REPS} server_cpus=${SERVER_CPUS} client_cpus=${CLIENT_CPUS}"
echo "concurrency=${CONNS}"
echo "oha=$("$OHA" --version)  fd_limit=$(ulimit -n)"
...
for target in "${TARGETS[@]}"; do
  ...
  for route in /json /users/42; do
    for rep in $(seq 1 "$REPS"); do
      run_cell "$target" "$route" 128 "$rep" "$port"
    done
  done
